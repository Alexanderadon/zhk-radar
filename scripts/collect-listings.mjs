...
// Город задаётся аргументами: node collect-listings.mjs <слуг> <N> <W> <S> <E>
...
async function page(n, w, s, e, p) {
  const url = `https://krisha.kz/a/ajax-map-list/map/prodazha/kvartiry/${CITY_SLUG}/?bounds=${n.toFixed(4)},${w.toFixed(4)},${s.toFixed(4)},${e.toFixed(4)}&page=${p}`;
...
await scrape(BBOX[0], BBOX[1], BBOX[2], BBOX[3], 0);
...
if (existsSync(`data/listings${SUFFIX}.json`)) {
  try {
    const prev = JSON.parse(await readFile(`data/listings${SUFFIX}.json`, 'utf-8'));
    ...
    if (existsSync(`data/listings-sold${SUFFIX}.json`)) { try { soldRecent = JSON.parse(await readFile(`data/listings-sold${SUFFIX}.json`, 'utf-8')); } catch {} }
...
await writeFile(`data/listings${SUFFIX}.json`, JSON.stringify(list), 'utf-8');
await writeFile(`data/listings-sold${SUFFIX}.json`, JSON.stringify(soldRecent), 'utf-8');
...
let intervalDays = 1;
try {
  const prevMeta = JSON.parse(await readFile(`data/listings-meta${SUFFIX}.json`, 'utf-8'));
...
await writeFile(`data/listings-meta${SUFFIX}.json`, JSON.stringify(meta, null, 1), 'utf-8');
